...
# Set to true to save weights of trained models
SAVE_WEIGHTS = False
...
def run(jobs_file, job_id=None, plot=False, echo_symlink_to=None, job_date=None):
    ...
    for params in jobs:
        ...
        if os.path.isfile(results_file) and plot:
            ...
            # AFTER DONE TRAINING SAVE AGENT MODELS FOR REUSE
            if SAVE_WEIGHTS:
                weights_file = '%s/%i' % (results_dir, job_id)
                weights_file += "-%s%i.wgts"
                if hasattr(agents[0].mod.model, 'parameters'):
                    torch.save(agents[0].mod.get_weights(), weights_file % ("mod", 1))
                if hasattr(agents[0].demod.model, 'parameters'):
                    torch.save(agents[0].demod.get_weights(), weights_file % ("demod", 1))
                if len(agents) > 1:
                    if hasattr(agents[1].mod.model, 'parameters'):
                        torch.save(agents[1].mod.get_weights(), weights_file % ("mod", 2))
                    if hasattr(agents[1].demod.model, 'parameters'):
                        torch.save(agents[1].demod.get_weights(), weights_file % ("demod", 2))
        # pr.disable()
