    ...
    for experiment_params in list_experiment_params:
        ...
        for train_snr in training_snrs:
            for trial in range(experiment_params['num_trials']):
                ...
                # trial_params['__meta__']['train_SNR_db'] = train_snr
                ...
                if a2:
                    if a2['mod_model'] in ['clone', 'selfalien']:
                        if a2['mod_model'] == 'clone':
                            # Sample any randomized mod parameters in a1 now so that they get copied exactly
                            deep_sample(a1['mod_params'], rs)
                        a2['mod_model'] = a1['mod_model']
                        a2['mod_params'] = deepcopy(a1['mod_params'])
                    if a2['demod_model'] in ['clone', 'selfalien']:
                        if a2['demod_model'] == 'clone':
                            # Sample any randomized demod parameters in a1 now so that they get copied exactly
                            deep_sample(a1['demod_params'], rs)
