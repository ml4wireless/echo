...
def load_lookup_table(home_dir = '.'):
    lookup_table_filename = home_dir + '/ber_lookup_table.pkl'
    with open(lookup_table_filename, 'rb') as f:
        lookup_table = pickle.load(f)
        return lookup_table
    
def save_lookup_table(lookup_table, home_dir = '.'):
    lookup_table_filename = home_dir + '/ber_lookup_table.pkl'
    ...
    def get_optimal_SNR_for_BER_roundtrip(self, target_ber_roundtrip, bits_per_symbol):
        ...
        if ckey not in self.seen_ber_roundtrip[bits_per_symbol]:
            ...
            if ck is not None:
                ...
                raise ValueError('a ber_roundtrip value below target ber_roundtrip was not achieved for any SNR_db in classics!')
        else:
    ...
    def get_optimal_SNR_for_SER_roundtrip(self, target_ser_roundtrip, bits_per_symbol):
        ...
        if ckey not in self.seen_ser_roundtrip[bits_per_symbol]:
            ...
            if ck is not None:
                ...
                raise ValueError('a ser_roundtrip value below target ser_roundtrip was not achieved for any SNR_db in classics!')
        else:
