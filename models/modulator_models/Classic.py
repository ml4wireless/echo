...
class Classic():
    def __init__(self,*, 
        ...
        **kwargs):
        ...
        self.rotation = torch.tensor(rotation).float()
    ...
    def rotate_symbol_map(self):
        mrotate = torch.tensor([[torch.cos(self.rotation), torch.sin(self.rotation)],
                            [-torch.sin(self.rotation), torch.cos(self.rotation)]])
        self.symbol_map = self.symbol_map @ mrotate
    ...
    def forward(self, symbols:torch.Tensor) -> torch.Tensor:
        ...
        return cartesian_points.to(symbols.device)
