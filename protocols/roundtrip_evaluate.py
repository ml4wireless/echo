...
def roundtrip_evaluate(*,
    ...
    for i in range(2):
        ...
        c_signal_forward = A.mod.modulate(preamble, mode='exploit', dtype='complex')
        ...
        for test_SNR_db in test_SNR_dbs:
            ...
            c_signal_backward = B.mod.modulate(preamble_halftrip, mode='exploit', dtype='complex')
            ...
            test_ber = get_ber(preamble, preamble_roundtrip)
            test_ser = get_ser(preamble, preamble_roundtrip)
            test_bers[i].append(float(test_ber))
            test_sers[i].append(float(test_ser))

        #Switch and find BER's other way
        A,B = B,A
    ...
    if verbose is True or iteration == num_iterations: #IF you want to manually debug:
        # print("Iter: %d, \t Means"%i)
        snr_db_padding = max(len(str(db)) for db in test_SNR_dbs)
        ber_padding = max(len(str(ber)) for ber in test_bers)
        print("Iter: %d,\n\t\t\t(%s --> %s --> %s), \n\t\t\t[%s --> %s --> %s], \n\t\t\t<Means>"%(iteration,A.name, B.name, A.name, B.name, A.name, B.name))
        
        print("Train SNR_db:% 5.1f"%SNR_db)
    ...
    return {
        ...
        'test_bers':test_bers, #mean
        'test_sers':test_sers, #mean
